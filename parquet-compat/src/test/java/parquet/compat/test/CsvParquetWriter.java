...
public class CsvParquetWriter extends ParquetWriter<List<String>> {
  // Reduce the block (row group) size for testing to expose more issues.
  public static final int TEST_BLOCK_SIZE = 4 * 1024 * 1024;
  ...
  public CsvParquetWriter(Path file, MessageType schema, CompressionCodecName codecName, boolean enableDictionary) throws IOException {
    super(file, (WriteSupport<List<String>>) new CsvWriteSupport(schema), codecName, TEST_BLOCK_SIZE, DEFAULT_PAGE_SIZE, enableDictionary, false);
  }
}
